class Instruction(object):
	...
	def decode(self,):
		...
		if opcode in self.opcode_dict:
			...
			#In the R case the Opcode = 6, Rs = 5 , Rt = 5 , Rd =5 and the rest 11 bits are unused total of 32 bits
			if self.opcode in self.r_type: 
				self.reg_rs = int(bin_inst[6:11],2)# 5 bits for rs 
				self.reg_rt = int(bin_inst[11:16],2)# 5 bits for rt
				self.reg_rd = int(bin_inst[16:21],2)# 5  bits for rd  , OPcode is the 6 MSB
				self.type = 'R'
			#  in the I case Opcode = 6, Rs = 5, Rt = 5, and Immediate = 16
			else:
				self.type = 'I'
				if self.opcode=='Halt':
					return
